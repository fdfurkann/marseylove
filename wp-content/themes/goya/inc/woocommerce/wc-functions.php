	...
	function goya_subcategories_by_id($parent_cat_ID) {
		...
		<ul class="shop_categories_list">
			<?php foreach ($subcats as $sc) {
				...
				<li><a href="<?php echo esc_url($link); ?>" class="cat-<?php echo esc_attr($sc->slug); ?>">
	...
	function goya_shop_filters() {
		...
		<div id="side-filters" class="side-panel <?php echo esc_attr(implode(' ', $panel_class)); ?>">
			<header>
				...
				<button class="et-close button btn-sm" aria-label="<?php esc_attr_e('Done', 'goya'); ?>"><?php esc_attr_e('Done', 'goya'); ?></button>
			</header>
...
---------------------------------------------------------- */
	...
	// Show thumbnail in order review at checkout
	add_filter( 'woocommerce_cart_item_name', 'goya_product_image_on_checkout', 10, 3 );
	 
	function goya_product_image_on_checkout( $name, $cart_item, $cart_item_key ) {
    if ( ! is_checkout() ) {
    	return $name;
    }
    ...
    $image = '<div class="et-product-thumbnail">' . $thumbnail . '</div>';
    $name =  '<span class="et-product-name">' . $name . '</span>'; 
...
---------------------------------------------------------- */
	function goya_product_badge() {
		...
		if ($stock_badge == true ) {
			...
			// Out of Stock simple/variable/composite products
			if ( ( $product->get_type() === 'simple' and !$product->is_in_stock() ) || ( $product->get_type() === 'variable' && !$product->is_in_stock() ) || ( $product->get_type() === 'composite' and !$product->is_in_stock() ) ) {
				$att_in_stock = false;
				?>
			...
			if($product->is_on_sale() && get_theme_mod('product_sale_flash', 'pct') != 'disabled' && get_theme_mod('shop_catalog_mode', false) == false ) {
				...
				if ( get_theme_mod('product_sale_flash', 'pct') == 'pct' ) {
					if ( $product->get_type() === 'variable' ) {
						...
						if ( $regular_price > 0 && $product->get_price()) {
...
---------------------------------------------------------- */
	...
	if ( get_theme_mod('product_share_buttons', true) == true ) {
		add_action('woocommerce_share', 'goya_social_share', 50);
	}
...
---------------------------------------------------------- */
	...
	function goya_sizing_guide_link() {
		...
		if (!empty($size_page)) { 
			if ($mode == 'lightbox') { ?>
			<a href="#sizing-popup" rel="inline" class="sizing-guide-open sizing_guide" data-class="et-sizing-guide" data-button-inside="true"><?php echo apply_filters('goya_size_guide_link_title', get_the_title($size_page)); ?></a>
		<?php } else { ?>
			<a href="<?php echo get_permalink($size_page); ?>" class="sizing-guide-open sizing_guide" data-class="et-sizing-guide" target="_blank"><?php echo apply_filters('goya_size_guide_link_title', get_the_title($size_page)); ?></a>
		<?php }
	}
	}
...
---------------------------------------------------------- */
	...
	if ( ! function_exists( 'goya_product_thumbnail_alt' ) ) {
		function goya_product_thumbnail_alt($product) {
			...
			if ($parent_id != 0) {
				...
				$gallery_thumbnail_ids = $parent->get_gallery_image_ids();
			}

			$thumbnail_alt_id = ( $gallery_thumbnail_ids ) ? reset( $gallery_thumbnail_ids ) : null; // Get first gallery image id

			if ( $thumbnail_alt_id ) {
				$thumbnail_alt_src = wp_get_attachment_image_src( $thumbnail_alt_id, 'woocommerce_thumbnail' );

				// Make sure the first image is found (deleted image id's can still be assigned to the gallery)
				if ( $thumbnail_alt_src ) {
					return wp_get_attachment_image( $thumbnail_alt_id, 'woocommerce_thumbnail', '', array('class'=>'product_thumbnail_hover') );
				}
			}
	...
	function goya_add_loop_variation_swatches() {
		...
		if (! $enable_swatches == true || is_admin()) {
			return;
		}
...
---------------------------------------------------------- */
	...
	function goya_single_product_layout_before() {
		...
		$gallery_classes[] = 'gallery-transition-' . $transition;
	...
	function goya_single_product_gallery_params( $params ) {
		...
		if ( isset( $params['flexslider'] ) ) {
			...
			$params['flexslider']['animationSpeed'] = ( $transition == 'fade' && $swap_hover == true ) ? 1 : 300;
	...
	add_filter('woocommerce_product_review_comment_form_args', 'goya_toggle_review_form');
	function goya_toggle_review_form($comment_form) {
	...
	/* Add container to short description */
	add_filter( 'woocommerce_short_description', 'goya_short_description_container' );
	function goya_short_description_container( $excerpt ){
	...
	function goya_product_tabs() {
		...
		function goya_woocommerce_remove_tabs( $tabs ) {
			...
			if ( get_theme_mod('product_accordion_swap_description', true) == true ) {
				$tabs['description']['callback'] = 'goya_short_desc_product_accordion_content';
		}
	...
	function goya_woocommerce_featured_video( $position ) {
		...
		if (!empty($video_url)) {
			...
			foreach ( $videos as $video ) {
				...
				preg_match('/^.*(?:(?:youtu\.be\/|v\/|vi\/|u\/\w\/|embed\/|shorts\/)|(?:(?:watch)?\?v(?:i)?=|\&v(?:i)?=))([^#\&\?]*).*/', $video, $match);
				if (!empty($match[1])) {
	...
	/* Buy Now button*/
	add_filter('woocommerce_add_to_cart_redirect', 'goya_add_redirect_to_checkout');
	function goya_add_redirect_to_checkout($redirect_url) {
	...
	add_action('woocommerce_after_add_to_cart_button', 'goya_add_buy_now_button', 1);
	function goya_add_buy_now_button() {
		global $product;
		if ( ! get_theme_mod('product_single_buy_now', false) == true ) {
			return;
		}
		$style = get_theme_mod('buy_now_style', 'outlined');
	 ?>
	 <div class="buy-now-container-single-product">
		 <div>
		<button type="submit" name="add-to-cart" value="<?php echo esc_attr($product->get_id()); ?>" class="single_buy_now_button et-buy-now-button button alt <?php echo esc_attr( $style ); ?>" id="buy_now_button">
		<?php echo esc_html__( 'Buy Now', 'goya' ); ?>
		</button>
