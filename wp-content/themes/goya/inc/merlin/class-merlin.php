...
class Merlin {
	...
	?>
		...
		<?php if ( $go == false) { ?>
			<p class="server_check_notice step_welcome"><?php echo sprintf( __( '%sAdjust your settings%s or proceed with caution', 'goya' ), '<a href="https://goya.everthemes.com/help-center/server-requirements/" target="_blank" class="red"><b>', '</b></a>' ); ?></p>
			<?php if ( $step == 'import' ) : ?>
				<?php $max_execution_time = intval(ini_get('max_execution_time'));
					if ($max_execution_time < 300 && $max_execution_time > 0 ) { ?>
				<p class="server_check_notice step_import"><?php echo sprintf( __( 'Demo thumbnails won\'t be regenerated to avoid errors (Your own uploaded images will resize as usual).', 'goya' ), '<span class="underline">', '</span>' ); ?></p>
				<?php } ?>
			<?php endif; ?>
		<?php } ?>
