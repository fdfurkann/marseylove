	...
	function wcapf_list_sub_terms($sub_term_args, $found) {
		...
		foreach ($sub_term_ids as $sub_term_id) {
			...
			if ($sub_term_data && ($sub_term_data->parent == $parent_term_id)) {
				$sub_term_ids_array[] = array(
					'term_id' => $sub_term_data->term_id,
					'name' => $sub_term_data->name,
				);
		...
		// sort by name
		$col = array_column( $sub_term_ids_array, "name" );
		array_multisort( $col, SORT_ASC, $sub_term_ids_array );


		foreach ($sub_term_ids_array as $sub_term_id) {
			//$sub_term_data = wcapf_get_term_data($sub_term_id, $taxonomy);
			//if ($sub_term_data && ($sub_term_data->parent == $parent_term_id)) {
				$_parent_term_id = $sub_term_id['term_id'];
				$_parent_term_name = $sub_term_id['name'];
			...
			foreach ($parent_terms as $parent_term) {
				...
				if ($count > 0 || $force_show === true) {
					...
					if (($enable_hierarchy === true && $show_children_only !== true) || ($show_children_only === true && (in_array($parent_term_id, $term_ids) || $force_show === true))) {
						...
						if (sizeof((array)$sub_term_ids) > 0) {
							$sub_term_args = array(
								'taxonomy'           => $taxonomy,
								'data_key'           => $data_key,
								'query_type'         => $query_type,
